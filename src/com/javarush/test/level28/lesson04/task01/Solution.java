...
В классе Solution создайте публичный статический класс AmigoThreadFactory, реализующий интерфейс
ThreadFactory
...
B - номер треда инкрементируется в пределах конкретной фабрики начиная с 1, используйте
AtomicInteger
...
public class Solution {
    ...
    public static class AmigoThreadFactory implements ThreadFactory {
        ...
        private AtomicInteger tmpInt = new AtomicInteger(0);
        public AmigoThreadFactory() {
            tmpInt.set(numberFabric.getAndIncrement());
        }
        public Thread newThread(Runnable e) {
            return new Thread(e, Thread.currentThread().getThreadGroup().getName() +
                    "-pool-" + tmpInt +
                    "-thread-" + numberThread.getAndIncrement());
        }
    }
}
